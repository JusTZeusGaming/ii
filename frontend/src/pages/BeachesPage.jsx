...
export default function BeachesPage() {
  ...
  return (
    <div className="px-5 py-6" data-testid="beaches-page">
        ...
        >
          ...
          <div className="horizontal-scroll">
            {recommendedBeaches.map((beach) => (
              <Card 
                key={beach.id}
                className="w-64 flex-shrink-0 rounded-2xl overflow-hidden bg-white border border-slate-100"
                data-testid={`beach-recommended-${beach.id}`}
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    >
                      Mappa <ExternalLink className="w-3 h-3 ml-1" />
                    </Button>
                  </div>
                </div>
              </Card>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filteredBeaches.map((beach, index) => (
            ...
            >
              <Card 
                className="flex overflow-hidden rounded-2xl bg-white border border-slate-100 interactive-card"
                ...
                <div className="flex-1 p-4">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => window.open(beach.map_url, "_blank")}
                      className="text-amber-600 hover:text-amber-700 p-0 h-auto text-xs"
                      data-testid={`beach-map-${beach.id}`}
                    >
                      Apri mappa
                    </Button>
                  </div>
      ...
      {filteredBeaches.length === 0 && !loading && (
        <div className="text-center py-12">
          <p className="text-slate-500">Nessuna spiaggia trovata</p>
        </div>
      )}
    </div>
  );
