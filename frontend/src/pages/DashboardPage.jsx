...
import { 
  Home, Umbrella, Utensils, Compass, Map, Bus, Bike,
  ChevronRight, ShoppingCart, Sun, Cloud, CloudRain, Wind
} from "lucide-react";
...
const WeatherIcon = ({ icon }) => {
  const icons = { "sun": Sun, "cloud": Cloud, "cloud-sun": Cloud, "cloud-rain": CloudRain, "cloud-drizzle": CloudRain, "cloud-fog": Cloud, "cloud-lightning": CloudRain };
...
export default function DashboardPage() {
  ...
  const quickActions = [
    { icon: Umbrella, label: t("dashboard.beaches"), to: "/spiagge", color: "bg-blue-500" },
    { icon: Utensils, label: t("dashboard.dining"), to: "/ristoranti", color: "bg-orange-500" },
    { icon: Compass, label: t("dashboard.activities"), to: "/attivita", color: "bg-emerald-500" },
    { icon: Map, label: t("dashboard.maps"), to: "/mappe", color: "bg-purple-500" },
    { icon: Bus, label: t("dashboard.transport"), to: "/trasporti", color: "bg-rose-500" },
    { icon: Bike, label: t("dashboard.rentals"), to: "/noleggi", color: "bg-amber-500" },
  ];
        ...
        >
          ...
          {weather && (
            <div 
              ...
              onClick={(e) => { e.stopPropagation(); navigate("/meteo"); }}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-5 text-white">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold">{t("dashboard.yourAccommodation")}</h2>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {["Wi-Fi", "Check-in/out", t("accom.houseRules"), t("accom.contacts"), "FAQ"].map((tag) => (
                <span key={tag} className="bg-white/20 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs font-medium">{tag}</span>
              ))}
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <p className="text-base font-bold text-slate-900">{t("dashboard.supermarket")}</p>
                <span className="bg-green-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{t("open")}</span>
              </div>
              <p className="text-sm text-slate-600 mt-0.5">{t("dashboard.supermarketDesc")}</p>
              <p className="text-xs text-green-600 font-medium mt-1">{t("dashboard.supermarketDistance")}</p>
            </div>
          </div>
        </Card>
      </motion.div>

      <motion.div className="mb-4" initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}>
        <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">{t("dashboard.explore")}</h3>
        <div className="grid grid-cols-2 gap-3">
          {quickActions.map((action, index) => {
            const Icon = action.icon;
            return (
              <motion.div key={action.to} initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.05 * index + 0.3 }}>
                <Card
